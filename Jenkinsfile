pipeline {
    ...
    stages {
        stage('Clone Repository') {
            steps {
                git 'https://github.com/ayush007-git/aws.git'
            }
        }
        ...
        stage('Deploy to AWS Ubuntu Server') {
            steps {
                ...
                sh '''
                scp -o StrictHostKeyChecking=no -r ./ ubuntu@13.60.57.125/:/var/www/myapp
                ssh ubuntu@13.60.57.125 'pm2 restart myapp || pm2 start /var/www/myapp/index.js --name myapp'
                '''
            }
        }
    }
